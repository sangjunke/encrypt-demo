...
function SM4Util() {
    ...
    this.encryptData_ECB = function (plainText) {
        try {
            ...
            sm4.sm4_setkey_enc(ctx, this.secretKey);
            var encrypted = sm4.sm4_crypt_ecb(ctx, stringToByte(plainText));
            var plaintext1 = Bytes2Str(encrypted);
            return plaintext1;
        } catch (e) {
            console.error(e);
            return null;
    ...
    stringToByte = function (str) { //字符串转为Unicode 码
        var bytes = new Array();
        var len, c;
        len = str.length;
        for (var i = 0; i < len; i++) {
            c = str.charCodeAt(i); //转为Unicode 码
            if (c >= 0x010000 && c <= 0x10FFFF) { //特殊字符如Ř，Ţ
